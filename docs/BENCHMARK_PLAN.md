...
│   ├── plot_peak_memory.py    # Fig 4
│   └── plot_feature_matrix.py # Fig 2 (checkmark grid)
└── results/
...
- **Figure 2** (feature matrix): one PNG / one PDF at 300 dpi. Checkmark grid, gfc column flagged with a subtle highlight. Generated by `analyze/plot_feature_matrix.py`.
